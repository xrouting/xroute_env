...
import models
from self_route import MCTS, Node, SelfRoute
...
class DiagnoseModel:
    ...
    def get_virtual_trajectory_from_obs(
        ...
        """
        XRoute routes a chip but uses its model instead of using the environment.
        ...
        root, mcts_info = MCTS(self.config).run(
            self.model, observation, self.config.action_space, True
        )
        trajectory_info.store_info(root, mcts_info, None, numpy.NaN)

        for i in range(horizon):
            action = SelfRoute.select_action(root, 0)
            ...
            root, mcts_info = MCTS(self.config).run(
                self.model, None, self.config.action_space, True, root
            )
    ...
    def compare_virtual_with_real_trajectories(
        self, first_obs, route, horizon, plot=True
    ):
        """
        First, XRoute routes a chip but uses its model instead of using the environment.
        Then, XRoute routes the optimal trajectory according precedent trajectory but performs it in the
        ...
        for i, action in enumerate(virtual_trajectory_info.action_history):
            ...
            if action not in route.legal_actions():
            ...
            observation, reward, done = route.step(action)
            root, mcts_info = MCTS(self.config).run(
                self.model,
                observation,
                route.legal_actions(),
